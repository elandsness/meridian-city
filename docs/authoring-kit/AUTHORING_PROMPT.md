...
1. **Correlation ID:** Every flow must have a correlation ID. For entity-driven flows, the default is `{entity_type}.id` (e.g., `service_request.id`). If a flow spans multiple entities or uses a custom identity, you MUST explicitly define the `correlationID` in the flow configuration. Never use generic names like `req_id` or `id`.
...
- `kind: shapes` — a schematic vector diagram drawn directly in the config, no image
  ...
  background:
    ...
    shapes:
      ...
      - { type: text, x: 180, "y": 185, text: "Seattle D

... [OUTPUT TRUNCATED - 14,713 chars omitted out of 64,640 total] ...
