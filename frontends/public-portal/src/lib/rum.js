...
// docs/INSTRUMENTATION.md — the property key matches the backend business event
// for the same journey so a RUM session can be joined to its funnel.

function dt() {
  return typeof window !== 'undefined' ? window.dtrum : undefined
}

// Tag the current session with the citizen identity (email preferred).
...
// Attach correlation properties (the join key to backend business events) to an
// in-flight action. Values are coerced to strings.
