...
// Configurable chat assistant widget for the ops dashboard.
...
export default function ChatWidget({ assistantName, assistantPersona, config }) {
  ...
  const name = assistantName || cfg.company?.assistant?.name || "Ops Assistant"
  const persona = assistantPersona || cfg.company?.assistant?.persona || "Meridian City's operations assistant"
      ...
      >
        <div className="bg-slate-800 px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2 text-white">
            <span className="w-6 h-6 rounded-full bg-amber-400 text-slate-900 flex items-center justify-center text-xs font-semibold">{name.charAt(0)}</span>
        ...
        <div className="border-t border-slate-200 p-3 flex gap-2 bg-white">
          <input
            ...
            disabled={loading}
            className="flex-1 bg-white border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-slate-500 disabled:opacity-50"
          />
          <button
            onClick={handleSend}
            disabled={loading || !input.trim()}
            className="bg-slate-800 hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-3 py-2 rounded-lg text-sm font-medium transition-colors"
          >
            Send
          </button>
        </div>
