...
// Schematic (not geographic) airfield. Aircraft flow vertically in a stable per-flight
// lane: gates (bottom) ↔ taxiway (middle) ↔ runway (top). Departures climb out the top,
// arrivals descend from it. Position is derived from flight status and smoothly
// interpolated toward the next node using time elapsed since updated_at, so planes glide
// continuously between the ~poll intervals (flight-ops resets progress each step, so we
// animate client-side rather than trust a 0..1 field).
const VW = 1000;
const VH = 620;
const SEG_MS = 55_000; // assumed time per status step (matches flight-lifecycle mid-band)
...
function depNode(state, laneX) {
  switch (state) {
    ...
    default: // at_gate / servicing / boarding — parked at the gate
...
function laneFor(id) {
  return 120 + (hashInt(id) % 760); // stable pseudo-random column in [120, 880]
}
...
function Airfield({ terminalName }) {
  return (
    <g>
      ...
      />
      <text x="72" y={Y_RUNWAY + 5} fill="#6b7280" fontSize="13" fontWeight="bold">
        27
      </text>
      <text x="912" y={Y_RUNWAY + 5} fill="#6b7280" fontSize="13" fontWeight="bold" textAnchor="end">
        09
      </text>

      {/* taxiway */}
      <line x1="100" y1={Y_TAXI} x2="900" y2={Y_TAXI} stroke="#facc15" strokeOpacity="0.22" strokeWidth="6" />
      <text x="104" y={Y_TAXI - 12} fill="#475569" fontSize="11" letterSpacing="1">
        TAXIWAY A
      </text>
      ...
      {/* gate stands along the terminal apron */}
      {Array.from({ length: 12 }).map((_, i) => {
        const gx = 168 + i * 60;
        return (
          <rect key={i} x={gx - 6} y={Y_GATE + 14} width="12" height="10" fill="#1f2937" stroke="#374151" />
        );
      })}
    </g>
  );
...
export default function AirfieldMap() {
  const cfg = useConfig();
  ...
  // Client tick drives the smooth glide between polls.
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Airfield</h1>
          <p className="text-gray-500 text-sm mt-1">Live aircraft positions — gate, taxiway &amp; runway.</p>
        </div>
        <div className="flex items-center gap-4 text-xs text-gray-400">
          <span className="flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 rounded-full bg-cyan-400" /> Departures
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 rounded-full bg-amber-400" /> Arrivals
          </span>
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
        <svg viewBox={`0 0 ${VW} ${VH}`} className="w-full" style={{ background: '#0b1220' }}>
          ...
          {isLoading && (
            <text x={VW / 2} y={VH / 2} fill="#64748b" textAnchor="middle" fontSize="16">
              Loading…
            </text>
          )}
        </svg>
