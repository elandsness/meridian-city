 ...
 *
 * <p>Lifted near-verbatim from service-dispatch's RoutingEngine.java (Stage 6 of
 * the generic-entity-engine initiative) into its own stateless service, called
 * over HTTP via a callService effect from the service_request entity type --
 * preserves the citizen-facing entity-service -> routing-service -> ops-facing
 * entity-service multi-hop trace the old citizen-service -> service-dispatch ->
 * city-operations chain had, and gives routing decisions their own
 * independently-Dynatrace-nameable service node.
 */
@Component
