...
async def _query_iot_incident_funnel(pool, stages: list[str]) -> list[dict]:
    async with pool.acquire() as conn:
        ...
        wo_total = await safe_fetchval(conn, "SELECT COUNT(DISTINCT ee.entity_id) FROM entities.entity_event ee JOIN entities.entity e ON e.id = ee.entity_id WHERE ee.entity_type = 'work_order' AND ee.event_type = 'work_order.created' AND e.links->>'incident_id' IS NOT NULL AND ee.occurred_at >= NOW() - ($1 || ' hours')::INTERVAL", WINDOW_HOURS)
        wo_assigned = await safe_fetchval(conn, "SELECT COUNT(DISTINCT ee.entity_id) FROM entities.entity_event ee JOIN entities.entity e ON e.id = ee.entity_id WHERE ee.entity_type = 'work_order' AND ee.event_type = 'work_order.assigned' AND e.links->>'incident_id' IS NOT NULL AND ee.occurred_at >= NOW() - ($1 || ' hours')::INTERVAL", WINDOW_HOURS)
        wo_acked = await safe_fetchval(conn, "SELECT COUNT(DISTINCT ee.entity_id) FROM entities.entity_event ee JOIN entities.entity e ON e.id = ee.entity_id WHERE ee.entity_type = 'work_order' AND ee.event_type = 'work_order.acknowledged' AND e.links->>'incident_id' IS NOT NULL AND ee.occurred_at >= NOW() - ($1 || ' hours')::INTERVAL", WINDOW_HOURS)
        wo_resolved = await safe_fetchval(conn, "SELECT COUNT(DISTINCT ee.entity_id) FROM entities.entity_event ee JOIN entities.entity e ON e.id = ee.entity_id WHERE ee.entity_type = 'work_order' AND ee.event_type = 'work_order.resolved' AND e.links->>'incident_id' IS NOT NULL AND ee.occurred_at >= NOW() - ($1 || ' hours')::INTERVAL", WINDOW_HOURS)
